...
function renderPredictions(data){
  ...
  container.innerHTML = "";
  if (!data.length){
    container.innerHTML = '<article class="prediction-card"><p>No predictions available right now.</p></article>';
    return;
  }
  data.forEach((p,index)=>{
    ...
    card.innerHTML = `
      ...
      ${kickoff ? `<p class="pill">${kickoff}</p>` : ""}
      <div class="panel"><p><strong>AI Analysis:</strong> ${p.reasoning || "No analysis available."}</p></div>
      <div class="chart-ring"><canvas id="chart${index}" aria-label="Confidence chart"></canvas></div>
      <p class="signal-label">${conf.label} • ${conf.units}</p>
      <a href="reviews.html" class="btn btn--primary">Check Best Bonus Route</a>
    `;
...
async function loadStats(){
  try{
    ...
    let running = 0;
    const values = [];
    labels.forEach((date)=>{ running += dailyProfit[date]; values.push(Number(running.toFixed(2))); });
    if (profitChartInstance) profitChartInstance.destroy();
    profitChartInstance = new Chart(profitCanvas.getContext("2d"), {
      type:"line",
      data:{labels, datasets:[{label:"Growth (Units)", data:values, borderColor:"#22d3ee", backgroundColor:"rgba(34,211,238,.12)", borderWidth:3, tension:.3, fill:true, pointRadius:0, pointHoverRadius:4}]},
      options:{responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{x:{ticks:{color:"#b8bfd3"}, grid:{color:"rgba(255,255,255,.08)"}}, y:{ticks:{color:"#b8bfd3"}, grid:{color:"rgba(255,255,255,.08)"}}}}
    });
  }catch(e){}
}
